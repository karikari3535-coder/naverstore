...
## 🔗 라이브 링크 (공유용) — 🟢 실데이터 연동 완료

**👉 https://c8fd1a71-1054-4597-a71e-f911648eda42.vip.gensparksite.com**

- **실제 네이버 쇼핑 검색 API**로 상위 40개 상품을 분석해 상품명을 추천합니다.
- 누구나 접속해 아무 키워드나 입력하면 진짜 데이터로 결과가 나옵니다.
- Cloudflare(Genspark 관리형 Workers)에 배포된 서버사이드 API를 통해
  API 키는 안전하게 서버에 숨겨져 있습니다 (프론트 노출 없음).

### API 엔드포인트
| 경로 | 설명 |
|---|---|
| `GET /` | 추천기 UI (demo.html) |
| `GET /healthz` | 헬스체크 → `ok` |
| `GET /api/status` | 실데이터 모드 여부 → `{"liveData":true}` |
| `GET /api/analyze?kw=신안새우&brand=&volume=` | 상위 40개 분석 → 추천 상품명·카테고리·키워드·난이도 |

### 데이터 소스
- ✅ **네이버 쇼핑 검색 API** (developers.naver.com) — 상위 상품명·카테고리·가격, 키워드 빈도 추출
- ⚠️ **검색광고 API** (키워드 검색량) — 미연동 (403). 현재는 **상위 상품 등장 빈도**로 추천 점수 계산.
- 키가 없을 경우 자동으로 더미 데이터(`DEMO_DB`)로 폴백합니다.
